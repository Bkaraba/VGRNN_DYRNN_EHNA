...
# building model
model = DynRNN(x_dim, h_dim, look_back=3, n_units=[h_dim]*n_layers, bias=True)
...
for k in range(1000):
    ...
    for i in range(seq_start, seq_end):
        if hidden_st is None:
            hidden_st = model.init_hidden()
        hidden_st = model(x_in[i], hidden_st)
    kld_loss, nll_loss = model.get_loss()
    ...
    if k > tst_after:
        hidden_st = None
        enc_means, pri_means = [], []
        for i in range(seq_end, seq_len):
            if hidden_st is None:
                hidden_st = model.init_hidden()
            _, pri_mean, hidden_st = model(x_in[i], hidden_st)
            enc_means.append(model.encoder.output[:, -1, :])
            pri_means.append(pri_mean)
        enc_means = torch.stack(enc_means, dim=0)
        pri_means = torch.stack(pri_means, dim=0)
